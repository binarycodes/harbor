...
- Tests live alongside the package they cover. Unit tests for the service layer, browserless tests for views, `*IT` Playwright tests for whole journeys. Both of the latter stub `MetadataResolver` so nothing reaches the network. Anything touching the library needs a real PostgreSQL — import `HarborDatabase` and empty the table in `@BeforeEach`, since one database now serves the whole suite. Every Spring test also needs `harbor.archive.browser-url` set, because the application refuses to start without a browser configured; those tiers stub `MetadataResolver` and never reach one. `BrowserPageArchiverTest` is the exception that does reach one: it starts a Chromium container through `ArchivingBrowser`, and `-Dharbor.archive.browser-url=…` points it at your own browser instead where containers cannot run.
