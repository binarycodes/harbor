...
- Login is mandatory. `SecurityConfig` leaves `VaadinSecurityConfigurer` on its defaults and points `oauth2LoginPage` at Keycloak, so an unauthenticated request redirects to the identity provider rather than to a Harbor login view — Harbor collects no credentials of its own.
- **A route with no access annotation is denied.** Navigation access control is on, so every `@Route` carries `@PermitAll`; adding a screen without one produces a route that resolves and then refuses. "Authenticated" is the whole authorization model — there are no roles.
- A logout that only drops Harbor's session leaves Keycloak's intact and the next visit signs straight back in. `OidcClientInitiatedLogoutSuccessHandler` is what makes the button mean what it says.
...
- Run build / test / frontend tasks through `./run.sh <task>` (`db`, `browser`, `keycloak`, `deps`, `compile`, `bundle`, `styles`, `test`, `verify`, `run`, `package`, `clean`), which pins JDK 21. Never invoke `mvn` directly. Every task but `deps` builds offline, so a newly added dependency needs `./run.sh deps` once. After a `@CssImport(themeFor=…)` / `@JsModule` change run `./run.sh bundle`; after editing an `@import`-ed CSS partial run `./run.sh styles`.
...
- **Every Spring test needs `@ActiveProfiles("test")`.** `src/test/resources/application-test.properties` carries what the context will not start without: the archiving browser (the application refuses to start with none configured) and an OAuth2 client (without a `ClientRegistrationRepository`, `oauth2LoginPage(...)` fails while the filter chain is being built). Both point at hosts that cannot resolve, and neither is ever called. A *profile* file deliberately — a plain `application.properties` under `src/test/resources` shadows the main one wholesale and takes the datasource and Flyway settings with it. Never give the OAuth2 client an `issuer-uri` here: that is fetched at startup, so it would make every test in the suite do OIDC discovery over the network.
...
- Two tests reach a real container of their own rather than stubbing. `BrowserPageArchiverTest` starts Chromium through `ArchivingBrowser`, and `-Dharbor.archive.browser-url=…` points it at your own browser instead where containers cannot run. `HarborJourneyIT` starts Keycloak through `HarborIdentity` and drives its login form, which is the only place the OAuth2 redirect, the token exchange, the subject and the logout are exercised for real — and which makes it a four-container test and the most fragile in the suite.
