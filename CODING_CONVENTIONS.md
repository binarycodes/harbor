...
- `AuthenticationContext` is the presenter-shaped API for who is signed in and for logging out, so a component may take it directly rather than through a Harbor presenter (`AccountMenu`) — the one sanctioned exception to §9, because a wrapper would hold no logic.
...
- `./run.sh env` brings up the whole development stack in one task and names no service: which containers Harbor needs is the stack definition's decision — `environment/dev/compose.yaml` under docker, the units in `environment/dev/quadlet` under podman — so adding one there needs no change to `run.sh`. podman gets quadlets rather than compose; `Notify=healthy` on every container is what makes systemd order them the way `depends_on: service_healthy` and `up --wait` did.
...
- Nothing in `run.sh` names Harbor: the project's answers are in `run.conf`, which the runner sources and then defaults everything the file leaves out, so another project reuses the runner as it stands. A task Harbor does not have goes in `run.tasks.sh` — a file of `task_<name>` functions, sourced after the helpers so they can call `setup` and `run_mvn`, dispatched by name, and listed under `help` through an optional `project_usage`. Keeping them out of `run.sh` is what lets a project take a newer runner without a merge; Harbor has no such file, and the hook costs nothing until it does. The `help` text therefore describes the runner and never the build — "enforces the JaCoCo coverage gate" or "Playwright tests" is a claim about Harbor's POM that is false in the next project — and `env` is left out of it entirely where `CONTAINER_REQUIRED` is not true, since advertising a task that then refuses is worse than not offering it.
